#!/usr/bin/env python3
"""
Mass generate round whiteboard magnet STL files from input images.

What it makes:
- round magnets of a fixed diameter
- fixed base thickness
- fixed image extrusion height
- a centered bottom recess sized for one 6x2 mm magnet
- STL output only
...
Notes:
- OpenSCAD must be installed because this script exports STL files.
- The image is raised on the front at a fixed height so you can set a single
  filament swap height in your slicer for every model.
- You can optionally use a JSON file with per-image threshold overrides.
"""
...
def build_scad(compounds: Sequence[CompoundPolygon], settings: Settings, model_name: str) -> str:
    ...
    scad = f"""// Auto-generated by magnet_generator.py
// Model: {model_name}
// Filament swap height: {settings.base_thickness:.2f} mm
